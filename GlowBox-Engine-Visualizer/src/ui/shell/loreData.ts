...
export const LORE_DATA: Record<string, IndexLore> = {
  ...
  rtree: {
    ...
    text: "It's 1984. Antonin Guttman at UC Berkeley is staring at a map and thinking: \"How on earth do you index a rectangle?\" Binary trees index points on a line. But the real world has shapes — polygons, regions, bounding boxes. His answer was the R-tree: a tree that groups nearby geometric objects into minimum bounding rectangles (MBRs), nesting them like Russian dolls all the way up to the root. It was imperfect — overlapping bounding boxes could force you to search multiple branches — but it was the first practical spatial index, and it launched an entire family of descendants (R*-tree, R+-tree, Hilbert R-tree). Today it powers map apps, GPS systems, and every \"find restaurants near me\" query you've ever made.",
    papers: [
  ...
  inverted: {
    ...
    text: "Long before Google, before AltaVista, before anyone called it \"search,\" librarians had a problem: how do you find every book that mentions a specific word without reading all of them? The answer, it turns out, was to flip the relationship inside out. Instead of document → words, you build word → documents. That inversion — so simple it feels obvious in hindsight — is the entire idea. The term \"inverted index\" appears in information retrieval literature as far back as the 1950s, but it was Gerard Salton's SMART system at Cornell in the 1960s–70s that turned it into a rigorous science, introducing concepts like TF-IDF weighting that are still used today. Then came the web: billions of pages, trillions of words. Doug Cutting built Lucene in 2000 as an open-source answer to industrial-scale full-text search, and its descendants — Solr and Elasticsearch — now quietly power the search bar on almost every website you use. Every time you type a query and get results in milliseconds, an inverted index somewhere is doing the heavy lifting. It is, without exaggeration, the index that made the internet navigable.",
    papers: [
