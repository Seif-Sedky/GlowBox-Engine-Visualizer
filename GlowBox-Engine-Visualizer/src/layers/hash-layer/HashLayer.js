...
export const HashLayer = () => {
    const treeState = useSessionStore(s => s.currentTreeState);
    const svgRef = useRef(null);
    const linksLayerRef = useRef(null);
    const dirLayerRef = useRef(null);
    const bucketsLayerRef = useRef(null);
    useEffect(() => {
        if (!treeState) {
            if (linksLayerRef.current)
                d3.select(linksLayerRef.current).selectAll('*').remove();
            if (dirLayerRef.current)
                d3.select(dirLayerRef.current).selectAll('*').remove();
            if (bucketsLayerRef.current)
                d3.select(bucketsLayerRef.current).selectAll('*').remove();
        }
    }, [treeState]);
    useEffect(() => {
            ...
            onDiff: (diff) => {
                ...
                if (diff.snapshot) {
                    ...
                    if (allDirNodes.length > 0) {
                        tl.to(allDirNodes, {
                            x: (i, el) => d3.select(el).datum().x,
                            y: (i, el) => d3.select(el).datum().y,
                            scale: 1,
                            opacity: 1,
                            duration: 0.5,
                            ease: 'power2.out'
